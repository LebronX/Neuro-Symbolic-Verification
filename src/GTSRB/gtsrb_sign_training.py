...
class SignNet(nn.Module):
    def __init__(self):
        ...
        self.fc_input = nn.Linear(resize * resize, 12)
        self.fc1 = nn.Linear(12, 12)  # 3*10: 2 sec, 3*20: 7 sec
        self.fc2 = nn.Linear(12, 12)
        # self.fc3 = nn.Linear(25, 25)
        self.fc_output = nn.Linear(12, 43)

        self.fc_input_prop = nn.Linear(resize * resize, 5)
        self.fc1_prop = nn.Linear(5, 5)  # 3 * 10 for hidden layers
        self.fc2_prop = nn.Linear(5, 5)
        # self.fc3_prop = nn.Linear(15, 15)
        self.fc_output_prop = nn.Linear(5, 2)
...
def train(args, model, device, train_loader, optimizer, epoch, sign):
    model.train()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # For property network
        # 14 is stop sign
        property_list = [1 if item == sign else 0 for item in target]
...
def test(model, device, test_loader, sign):
    model.eval()
    test_loss = 0
    correct = 0
    test_loss_prop = 0
...
def main():
    ...
    train_data_path = "../../TrainingData/GTSRB/Final_Training/images"
    ...
    train_loader = Data.DataLoader(train_data, **train_kwargs)
    test_loader = Data.DataLoader(val_data, **test_kwargs)
    ...
    for sign in range(10):
        for epoch in range(1, args.epochs + 1):
            train(args, model, device, train_loader, optimizer, epoch, sign)
            test(model, device, test_loader, sign)
            scheduler.step()
